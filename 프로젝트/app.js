...
function renderResult(result, forceExtras = false, scroll = true) {
  ...
  els.metrics.innerHTML = [["fa-right-left","환승횟수",`${transfers}<small>회</small>`],["fa-train-subway","총 이동역",`${stops - 1}<small>개 역</small>`],["fa-clock","현재 시간",formatScheduleTime(nowSeconds)],["fa-train-subway","다음 열차 출발","확인 중"],["fa-clock","총 이동시간",`${totalMinutes}<small>분</small>`],["fa-flag-checkered","최종 도착 예정",formatScheduleTime(nowSeconds + totalMinutes * 60)]].map(([icon,label,value]) => `<div class="metric glass-card"><i class="fa-solid ${icon}"></i><p class="metric-label">${label}</p><strong>${value}</strong></div>`).join("");
